...
class KalmanFilterBaselineModel(BaselineModel):
    ...
    def __init__(
        ...
        *,
        ...
        self._impl: classic_baseline.KalmanFilter | None = None

    def calibrate(
        self,
        calibration_file: str | None = None,
    ) -> dict:
        self._impl = classic_baseline.KalmanFilter(
    ...
    def _predict_block(self, seq_latlon_t: np.ndarray) -> np.ndarray:
        if self._impl is None:
            raise RuntimeError("KalmanFilterBaselineModel is not initialized.")
        lat = seq_latlon_t[:, 0]
        lon = seq_latlon_t[:, 1]
